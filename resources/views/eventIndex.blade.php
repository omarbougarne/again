<x-main-layout>
    <form method="GET" action="{{ route('eventFilter') }}">
        ...
        <div class="flex items-center">
            <label for="category" class="block mr-2 text-white">Filter by Category:</label>
            <select name="category" id="category" class="border rounded-md p-1 w-36">
            ...
            <div class="grid grid-cols-1 gap-8 mt-8 md:mt-16 md:grid-cols-2">
                @foreach ($events as $event)
                    <div class="lg:flex bg-slate-100 rounded-md text-black">
                        <img src="{{ asset('/images/' . $event->image) }}" style="max-height: 300px;" alt="Event Image"/>
